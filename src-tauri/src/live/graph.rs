...
pub struct SceneGraph {
    ...
    overlay: Option<(*mut ffi::obs_sceneitem_t, *mut ffi::obs_source_t, OverlaySpec)>,
}
    ...
    pub fn set_overlay(&mut self, spec: OverlaySpec) -> Result<(), String> {
        unsafe {
            ...
            let (src, color_key) = match &spec {
                ...
                OverlaySpec::Window { id: window_id, color_key } => {
                ...
                OverlaySpec::Browser { url } => {
                    ...
                    ffi::obs_data_set_int(settings, CString::new("width").unwrap().as_ptr(), bw as i64);
                    ffi::obs_data_set_int(settings, CString::new("height").unwrap().as_ptr(), bh as i64);
                    // overlay audio joins the mix as a source, not the desktop
                    ffi::obs_data_set_bool(settings, CString::new("reroute_audio").unwrap().as_ptr(), true);
