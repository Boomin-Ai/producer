...
impl Live {
    ...
    #[cfg(have_engine)]
    pub fn set_overlay(&self, window_id: Option<u32>, color_key: bool, url: Option<String>) -> Result<(), String> {
        let spec = match (url, window_id) {
            (Some(url), _) if !url.trim().is_empty() => graph::OverlaySpec::Browser { url: url.trim().into() },
            (_, Some(id)) => graph::OverlaySpec::Window { id, color_key },
            _ => graph::OverlaySpec::None,
        };
        self.handle.as_ref().ok_or("live engine not running")?.set_overlay(spec)
    }
    #[cfg(not(have_engine))]
